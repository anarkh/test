...
Plan.prototype.control = function () {
  const _this = this
  const stage = $('.gamewrap')
  let move = false
  stage.on('touchstart', function (event) {
    const x = event.changedTouches[0].clientX - offsetLeft
...
function EnemyPlan (canvas, options) {
  ...
  this.ctx = canvas
  this.o = $.extend(o, options)
  const randomx = parseInt(Math.random() * (this.ctx.canvas.clientWidth - this.o.w))
  this.o.x = randomx
  this.o.y = -this.o.h
  this.init()
}
...
function Gamedfj (options) {
  ...
  this.o = $.extend(o, options)
  const ww = $(window).width() > 360 ? 360 : $(window).width()
  const wh = $(window).height() > 640 ? 640 : $(window).height()
...
Gamedfj.prototype.init = function () {
  const _this = this
  const canvas = document.getElementById(_this.o.id)
...
Gamedfj.prototype.main = function () {
  ...
  let speed = parseInt(_this.o.score / 60) + 1
  ...
  let enemyPlanlen = _this.o.enemyPlanList.length
  for (var i = 0; i < enemyPlanlen; i++) {
    const enemyPlan = _this.o.enemyPlanList[i]
    enemyPlan.move()
    ...
    for (let bi = 0; bi < bulletslen; bi++) {
      const bullet = _this.o.bulletList[bi]
