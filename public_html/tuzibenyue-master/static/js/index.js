function Ship (ctx) {
  ...
  this.controll = function () {
    ...
    const currentX = this.left
    const currentY = this.top
    let move = false
...
function Food (type, left, id) {
  ...
  const p = this.type == 0 ? 'static/img/food1.png' : 'static/img/food2.png'
...
var gameMonitor = {
  ...
  genorateFood: function () {
    const genRate = 50 // 产生月饼的频率
    const random = Math.random()
    if (random * genRate > genRate - 1) {
      const left = Math.random() * (this.w - 50)
      const type = Math.floor(left) % 2 == 0 ? 0 : 1
      const id = this.foodList.length
      const f = new Food(type, left, id)
  ...
  getScore: function () {
    const time = Math.floor(this.time / 60)
