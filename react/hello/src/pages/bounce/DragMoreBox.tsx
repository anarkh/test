...
export const DragMoreBox = (props: DragMoreBoxProps) => {
  const { onMore, defaultText = '查看更多', draggingText = '松开查看', paddingLeft, dragCardStyles = {}, dragBgColor = '#f5f5f5' } = props;
  ...
  const onTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    const scrollDistance = listEl.offsetWidth + listEl.scrollLeft - listEl.scrollWidth;
    if (scrollDistance > 1) {
      ...
      const d =
        ...
        dragDistance < 0 ? 0 : dragDistance > DRAG_MAX ? DRAG_MAX : dragDistance; // 受限的距离
      // 函数处理 拉动越来越慢
      const transD = DISTANCE_MAX - (DISTANCE_MAX / DRAG_MAX ** 2) * (d - DRAG_MAX) ** 2;
  ...
  return (
    <div
      ...
      <div ref={scrollRef} className="card-list" style={{ paddingLeft }}>
        <div className="before" style={{ background: dragBgColor }} />
        {props.children}
        <div className="more-wrap">
          <div
            ...
            <div className={classNames('more-icon', dragging ? styles.iconRotate : '')} style={{ width: '15px', height: '15px' }} />
            <div className="more-text">{dragging ? draggingText : defaultText}</div>
          </div>
        </div>
        <div className="after" style={{ background: dragBgColor }} />
      </div>
    </div>
  );
};

interface DragMoreListOption<T> extends DragConfigProps {
  list: T[];
...
function DragMoreList<T>(props: DragMoreListOption<T>) {
  const { list, renderCard, spacing, disableDrag = false, ...innerProps } = props;

  // 卡片列表
  const inner = (
    <React.Fragment>
      {list.map((item: T, index) => (
        <div key={index} className={styles.cardWrapper} style={{ paddingRight: spacing }}>
          {renderCard(item)}
        </div>
      ))}
    </React.Fragment>
  ...
  return (
    <div className="drag-more-list-component">
      {disableDrag ? (
        <div className="card-list" style={{ paddingLeft: innerProps.paddingLeft }}>
          {inner}
        </div>
      ) : (
